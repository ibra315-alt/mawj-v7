...
export function Btn({ children, variant='primary', size='md', loading, onClick, style, type='button', disabled }) {
  ...
  const vars = {
    primary:   { bg:'linear-gradient(135deg,#00e4b8,#00c49a)', color:'#07090f', fw:800, shadow:'0 4px 18px rgba(0,228,184,0.35)', border:'none' },
    secondary: { bg:'var(--bg-glass)', color:'var(--text-sec)', fw:600, shadow:'none', border:'1.5px solid var(--bg-border)', backdropFilter:'blur(12px)' },
    danger:    { bg:'linear-gradient(135deg,#ff4757,#cc1020)', color:'#fff', fw:700, shadow:'0 4px 18px rgba(255,71,87,0.35)', border:'none' },
    ghost:     { bg:'transparent', color:'var(--text-sec)', fw:600, shadow:'none', border:'1.5px solid var(--bg-border)' },
    violet:    { bg:'linear-gradient(135deg,#8b5cf6,#6d28d9)', color:'#fff', fw:700, shadow:'0 4px 18px rgba(139,92,246,0.4)', border:'none' },
  }
  ...
  return (
    <button
      ...
      style={{
        ...
        border: v.border,
        background: off ? 'rgba(255,255,255,0.06)' : v.bg,
...
/* ══════════════════════════════════════════════
   CARD — hover via CSS class only, zero JS
══════════════════════════════════════════════ */
export function Card({ children, style, hover, glow, onClick }) {
  return (
    <div
      onClick={onClick}
      className={hover ? 'mawj-card mawj-card-hover' : 'mawj-card'}
      style={{
...
export function StatCard({ label, value, color, trend, sub, icon }) {
  ...
  return (
    <div
      className="mawj-stat-card hover-lift"
      style={{
      ...
      <div style={{
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 20px', borderBottom:'1px solid var(--bg-border)', flexShrink:0, background:'var(--modal-bg)' }}>
          <button
            onClick={onClose}
            className="icon-btn"
            style={{ background:'var(--bg-glass)', border:'1.5px solid var(--bg-border)', borderRadius:'var(--radius-pill)', width:36, height:36, cursor:'pointer', color:'var(--text-sec)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:15, flexShrink:0, WebkitTapHighlightColor:'transparent' }}
          >✕</button>
...
export function ColorPicker({ value, onChange }) {
  return (
    <div style={{ position:'relative', width:44, height:44 }}>
      <div style={{ width:44, height:44, borderRadius:'var(--radius-sm)', background:value, border:'2px solid var(--bg-border)', cursor:'pointer', boxShadow:`0 0 14px ${value}55` }} />
