...
async function callAI(messages: any[], systemPrompt: string, model: string, maxTokens = 1500, apiKeys: any = {}) {
...
async function executeTool(name, params, undoStack, dryRun = false) {
  ...
  switch (name) {
    ...
    case 'get_summary': {
      ...
      const bd = (o: any, from: Date, to?: Date) => { const d = new Date(o.order_date || o.created_at); return d >= from && (!to || d < to) }
    ...
    case 'reconcile_hayyak_cod': {
      ...
      const suggestions = openRem.slice(0, 10).map(rem => {
        ...
        const nearby = unlinked.filter(o => Math.abs(new Date(o.order_date || o.created_at).getTime() - remDate.getTime()) / 86400000 <= 7)
    ...
    case 'detect_anomalies': {
      ...
      const monthS  = new Date(now.getFullYear(), now.getMonth(), 1)
      const bd = (o: any, f: Date, t?: Date) => { const d = new Date(o.order_date || o.created_at); return d >= f && (!t || d < t) }
      const todayO = orders.filter(o => bd(o, todayS))
    ...
    case 'generate_whatsapp_report': {
      ...
      const topCities = Object.entries(cityMap as Record<string,number>).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([c, n]) => `${c}: ${n}`).join(' | ')
      const lines = [
    ...
    case 'analyze_system': {
      ...
      const stuckOrders = orders.filter(o => {
        ...
        const days = (now.getTime() - new Date(o.order_date || o.created_at).getTime()) / 86400000
        return days > 7
      })
      ...
      const topCity = Object.entries(orders.reduce((acc: Record<string,number>, o: any) => { const c = o.customer_city || 'أخرى'; acc[c] = (acc[c] || 0) + 1; return acc }, {} as Record<string,number>)).sort((a, b) => (b[1] as number) - (a[1] as number))[0]
...
const S = {
  ...
  input: { padding: '9px 13px', background: 'var(--bg-hover)', border: '1.5px solid var(--input-border)', borderRadius: 'var(--r-md)', color: 'var(--text)', fontSize: 13, fontFamily: 'inherit', outline: 'none', width: '100%', boxSizing: 'border-box' as const },
}
...
// ── Main Component ─────────────────────────────────────────────
export default function AgentPage({ user, onNavigate }: PageProps) {
  ...
  // ── Agentic loop ─────────────────────────────────────────────
  const runAgent = useCallback(async (taskText?: any, label?: any) => {
              ...
              {editingMem === m.id ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button onClick={() => saveMem(m.id, (document.getElementById(`key-${m.id}`) as HTMLInputElement)?.value, (document.getElementById(`val-${m.id}`) as HTMLInputElement)?.value, parseInt((document.getElementById(`imp-${m.id}`) as HTMLInputElement)?.value))} style={{ ...S.btn('primary'), flex: 1 }}>حفظ</button>
